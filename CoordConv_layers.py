...
class AddCoords(nn.Module):
    def __init__(self, rank, with_r=True, use_cuda=True):
...
class CoordConv2d(conv.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, with_r=True, use_cuda=False):
...
class CoordConvTranspose2d(conv.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
...
def Coordconv_block(in_dim, out_dim, act_fn, kernel_size=3, stride=1, padding=1, dilation=1):
    model = nn.Sequential(
        CoordConv2d(in_dim, out_dim, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation),
...
def Coordconv_decod_block(in_dim, out_dim, act_fn):
    model = nn.Sequential(CoordConvTranspose2d,
        nn.BatchNorm2d(out_dim),
        act_fn,
    )
    return model
...
class CoordConv_residual_conv(nn.Module):
    def __init__(self, in_dim, out_dim, act_fn):
        ...
        self.conv_1 = Coordconv_block(self.in_dim, self.out_dim, act_fn)
        self.conv_2 = Coordconv_block(self.out_dim, self.out_dim, act_fn)
        self.conv_3 = Coordconv_block(self.out_dim, self.out_dim, act_fn)
