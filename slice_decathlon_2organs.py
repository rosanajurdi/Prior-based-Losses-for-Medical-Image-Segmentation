...
def save_slices(IM_path, gt_path,
    ...
    #with open(os.path.join(str(list(IM_path.parents)[0]).split('/')[-4],'size.txt'), 'w') as the_file:
    #    f = (gt == 1).sum(axis=(0,1))
    ...
    for j in range(flair.shape[-1]):
        ...
        if discard_negatives and (gt_s.sum() == 0) and str(dest_dir).split('/')[-1] == 'train':
            continue
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--source_dir', type=str, default='/media/eljurros/Transcend/Decathlone/Task07_Pancreas/Task07_Pancreas/nifty/FOLD_3/nifty')
    parser.add_argument('--dest_dir', type=str, default='/media/eljurros/Transcend/Decathlone/Task07_Pancreas/Task07_Pancreas/nifty/FOLD_3/npy')
    ...
    parser.add_argument('--discard_negatives', action='store_true', default = True)
