...
def do_epoch(mode: str, net: Any, device: Any, loader: DataLoader, epc: int,
    ...
    for j, data in tq_iter:
        ...
        mean_dict = {"DSC": all_dices[big_slice, metric_axis].mean(),
                     "DSC1": all_dices[big_slice, metric_axis[0]].mean(),
                     "DSC2": all_dices[big_slice, metric_axis[1]].mean(),
...
def run(args: argparse.Namespace) -> None:
    ...
    metrics = {"val_dice": torch.zeros((n_epoch, n_val, n_class), device=device).type(torch.float32),
               #"val_dice_S1": torch.zeros((n_epoch, n_val, n_class), device=device).type(torch.float32),
               #"val_dice_S2": torch.zeros((n_epoch, n_val, n_class), device=device).type(torch.float32),
    ...
    for i in range(n_epoch):
        ...
        df = pd.DataFrame({"tra_loss": metrics["tra_loss"].mean(dim=1).cpu().numpy(),
                           ...
                           "val_dice": metrics["val_dice"][:, :, [-1,-2]].mean(dim=1).mean(dim=1).cpu().numpy(),
                           #"tra_batch_dice": metrics["tra_batch_dice"][:, :, -1].mean(dim=1).cpu().numpy(),
                           #"val_batch_dice": metrics["val_batch_dice"][:, :, -1].mean(dim=1).cpu().numpy(), 
                           'val_dice_S1':  metrics["val_dice"][:, :, [-1,-2]].mean(dim=1).cpu().numpy()[:,1], 
                           'val_dice_S2':  metrics["val_dice"][:, :, [-1,-2]].mean(dim=1).cpu().numpy()[:,0],
                           
                           })
        df.to_csv(Path(savedir, args.csv), float_format="%.7f", index_label="epoch")
        print(df)
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--dataset', type=str, default='/media/eljurros/Transcend/Decathlone/Task04_Hippocampus/FOLD_2/npy')
    parser.add_argument("--csv", type=str, default= 'metrics.csv')
    parser.add_argument("--workdir", type=str, default = '/media/eljurros/Transcend/Decathlone/Task04_Hippocampus/FOLD_2/results/gmd')
    ...
    parser.add_argument("--losses", type=str, 
                        ...
                        default="[('contour_loss', {'idc': [2]}, None, None, None, 1)]")
    ...
    parser.add_argument("--scheduler", type=str, default="DummyScheduler")#"StealWeight"
