...
for loss in ['Dice']:
    ...
    for n, Label in enumerate(['prostateP1', 'isles','prostateP2',  'hippoH1','hipooH2','acdc-H1', 'acdc-H2', 'acdc-H3', 'spleen', 'atrium']):
        ...
        polyfit_x.append(np.mean(sampled_size))
        polyfit_y.append(np.mean(sampled_mean))

        ax2.plot(np.mean(sampled_size), np.mean(sampled_mean), 'o', c=COLOR_note[Label])
        ax2.errorbar(np.mean(sampled_size), np.mean(sampled_mean), yerr=np.std(sampled_mean),xerr=np.std(sampled_size), fmt='-o', c=COLOR_note[Label])
        ax2.errorbar(np.mean(sampled_size), np.mean(sampled_mean), yerr=np.std(sampled_mean),xerr=np.std(sampled_size), fmt='-o', c=COLOR_note[Label])
    ...
    polyfit_y.append(np.mean(sampled_mean))
    ax2.plot(np.mean(sampled_size), np.mean(sampled_mean), 'o', c=COLOR_note[Label])
    ax2.errorbar(np.mean(sampled_size), np.mean(sampled_mean), yerr=np.std(sampled_mean), xerr=np.std(sampled_size),
                 fmt='-o', c=COLOR_note[Label])

    m, b = np.polyfit(polyfit_x, polyfit_y, 1)
    plt.plot(polyfit_x,  m*np.array(polyfit_x)+ b)
    plt.savefig('/Users/rosana.eljurdi/PycharmProjects/Prior-based-Losses-for-Medical-Image-Segmentation/resources/Meta-features/{}_regression'.format(loss))
# ax.legend()
#ax.grid(True)
